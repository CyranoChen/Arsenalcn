...
namespace Arsenalcn.CasinoSys.DataAccess
{
    public class Group
    {
        ...
        public static DataRow GetGroupTeamInfo(Guid groupGuid, Guid teamGuid, SqlTransaction trans)
        {
            ...
            DataSet ds;
            if (trans == null)
                ds = SqlHelper.ExecuteDataset(SQLConn.GetConnection(), CommandType.Text, sql, new SqlParameter("@groupGuid", groupGuid), new SqlParameter("@teamGuid", teamGuid));
            else
                ds = SqlHelper.ExecuteDataset(trans, CommandType.Text, sql, new SqlParameter("@groupGuid", groupGuid), new SqlParameter("@teamGuid", teamGuid));
