        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            if (ConfigGlobal.PluginActive && ConfigGlobal.PluginContainerActive && ProfileUserID != -1)
            {
                ...
                if (clubs.Count != 0)
                {
                    ...
                    var strClub = string.Format("<div class=\"{4}\" title=\"{5}\"><a href=\"plugin/AcnClub/ClubView.aspx?clubID={0}\" target=\"_blank\">{1}</a><em>LV:{2} | RPos:{3}</em></div>", club.ID.Value.ToString(), club.FullName, club.RankLevel.Value.ToString(), club.RankScore.Value.ToString(), cClubCSS, cClubTitle);
                    Response.Write($"document.write('{strClub.ToString()}');");
                }
                ...
                {
                    var strPlayer =
                        $"GenSwfObject('UserStrip', 'plugin/acnclub/swf/UserStrip.swf?XMLURL=plugin/acnclub/ServerXml.aspx%3FUserID={ProfileUserID.ToString()}', '180', '120');";
