...
namespace Arsenalcn.Common.DataAccess
{
    public class Config
    {
        ...
        {
            var sql =
                "SELECT ConfigSystem, ConfigKey, ConfigValue FROM dbo.Arsenalcn_Config WHERE ConfigSystem = @configSystem AND ConfigKey = @configKey";
        ...
        {
            var sql =
                "UPDATE dbo.Arsenalcn_Config SET ConfigValue = @configValue WHERE ConfigSystem = @configSystem AND ConfigKey = @configKey";

            SqlParameter[] para =
            {
                new SqlParameter("@configSystem", configSystem),
                new SqlParameter("@configKey", configKey), new SqlParameter("@configValue", configValue)
            };
        ...
        {
            var sql =
                "INSERT INTO dbo.Arsenalcn_Config (ConfigSystem, ConfigKey, ConfigValue) VALUES (@configSystem, @configKey, @configValue)";
