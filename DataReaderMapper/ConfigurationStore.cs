...
using DataReaderMapper.Configuration;
...
namespace DataReaderMapper
{
    public class ConfigurationStore : IConfigurationProvider
    {
        ...
        public INamingConvention SourceMemberNamingConvention
        {
            ...
            {
                GetProfile(DefaultProfileName)
                    .DefaultMemberConfig.AddMember<NameSplitMember>(_ => _.SourceMemberNamingConvention = value);
            }
        }
        ...
        public INamingConvention DestinationMemberNamingConvention
        {
            get
            {
                INamingConvention convention = null;
                GetProfile(DefaultProfileName)
                    .DefaultMemberConfig.AddMember<NameSplitMember>(
                        _ => convention = _.DestinationMemberNamingConvention);
                return convention;
            }
            set
            {
                GetProfile(DefaultProfileName)
                    .DefaultMemberConfig.AddMember<NameSplitMember>(_ => _.DestinationMemberNamingConvention = value);
            }
        }
        ...
        public IEnumerable<IMemberConfiguration> MemberConfigurations
            => GetProfile(DefaultProfileName).MemberConfigurations;
        ...
        public IEnumerable<IConditionalObjectMapper> TypeConfigurations
            => GetProfile(DefaultProfileName).TypeConfigurations;
        ...
        public void Seal()
        {
            ...
            foreach (var typeMap in _userDefinedTypeMaps.Values)
            {
                ...
                {
                    redirectedTypes.Add(Tuple.Create(typeMap.Types,
                        new TypePair(typeMap.SourceType, typeMap.DestinationTypeOverride)));
                }
                derivedMaps.AddRange(
                    GetDerivedTypeMaps(typeMap)
                        .Select(
                            derivedMap =>
                                Tuple.Create(new TypePair(derivedMap.SourceType, typeMap.DestinationType), derivedMap)));
            }
        ...
        public IMappingExpression CreateMap(Type sourceType, Type destinationType, MemberList memberList,
            string profileName)
        {
            if (TypeExtensions.IsGenericTypeDefinition(sourceType) &&
                TypeExtensions.IsGenericTypeDefinition(destinationType))
            {
        ...
        {
            return ResolveTypeMap(resolutionResult.Type, destinationType) ??
                   ResolveTypeMap(resolutionResult.MemberType, destinationType);
        }
