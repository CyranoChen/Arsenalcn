...
namespace DataReaderMapper.Internal
{
    ...
    public class AllMemberInfo : IGetTypeInfoMembers
    {
        ...
        {
            return
                typeInfo.PublicReadAccessors.Concat(typeInfo.PublicNoArgMethods)
                    .Concat(typeInfo.PublicNoArgExtensionMethods)
                    .ToList();
        }
    }
    ...
    public class PrePostfixName : ISourceToDestinationNameMapper
    {
            ...
            Type destType, string nameToSearch)
        {
            ...
            var possibleDestNames =
                getTypeInfoMembers.GetMemberInfos(typeInfo)
                    .Select(mi => new {mi, possibles = PossibleNames(mi.Name, Prefixes, Postfixes)});
        ...
        private IEnumerable<string> PossibleNames(string memberName, IEnumerable<string> prefixes,
            IEnumerable<string> postfixes)
        {
            ...
            foreach (
                var withoutPrefix in
                    prefixes.Where(prefix => memberName.StartsWith(prefix, StringComparison.Ordinal))
                        .Select(prefix => memberName.Substring(prefix.Length)))
            {
    ...
    public class ReplaceName : ISourceToDestinationNameMapper
    {
            ...
            Type destType, string nameToSearch)
        {
            ...
            var possibleDestNames =
                getTypeInfoMembers.GetMemberInfos(typeInfo).Select(mi => new {mi, possibles = PossibleNames(mi.Name)});
    ...
    public class SourceToDestinationNameMapperAttributesMember : ISourceToDestinationNameMapper
    {
        private static readonly
            IDictionary<TypeDetails, Dictionary<MemberInfo, IEnumerable<SourceToDestinationMapperAttribute>>> Cache =
                new DictionaryFactory()
                    .CreateDictionary
                    <TypeDetails, Dictionary<MemberInfo, IEnumerable<SourceToDestinationMapperAttribute>>>();
    ...
    public interface IMemberConfiguration
    {
        ...
        bool MapDestinationPropertyToSource(IProfileConfiguration options, TypeDetails sourceType, Type destType,
    ...
    public class MemberConfiguration : IMemberConfiguration
    {
        ...
        private TMemberMapper GetOrAdd<TMemberMapper>(Func<IMemberConfiguration, IList> getList,
    ...
    public class NameSplitMember : IChildMemberConfiguration
    {
        ...
        public IEnumerable<MethodInfo> SourceExtensionMethods
            => (Mapper.Configuration as ConfigurationStore).SourceExtensionMethods;
        ...
        {
            var first = string.Join(SourceMemberNamingConvention.SeparatorCharacter,
                matches.Take(i)
                    .Select(
                        s =>
                            SourceMemberNamingConvention.SplittingExpression.Replace(s,
                                SourceMemberNamingConvention.ReplaceValue))
                    .ToArray());
            var second = string.Join(SourceMemberNamingConvention.SeparatorCharacter,
                matches.Skip(i)
                    .Select(
                        s =>
                            SourceMemberNamingConvention.SplittingExpression.Replace(s,
