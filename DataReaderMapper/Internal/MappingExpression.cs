...
namespace DataReaderMapper.Internal
{
        ...
        IMemberConfigurationExpression
    {
        public MappingExpression(TypeMap typeMap, Func<Type, object> typeConverterCtor,
            IProfileExpression configurationContainer) : base(typeMap, typeConverterCtor, configurationContainer)
        {
        }

        public new IMappingExpression ReverseMap()
        {
            var mappingExpression = ConfigurationContainer.CreateMap(TypeMap.DestinationType, TypeMap.SourceType,
                MemberList.Source, TypeMap.Profile);
        ...
        public void ConvertUsing(Type typeConverterType)
        {
            ...
            var convertMethodType = interfaceType.IsAssignableFrom(typeConverterType)
                ? interfaceType
                : typeConverterType;
        ...
        public new IMappingExpression ForSourceMember(string sourceMemberName,
        ...
        public new IMappingExpression ForCtorParam(string ctorParamName,
        ...
        IMemberConfigurationExpression<TSource>
    {
        ...
        public IProfileExpression ConfigurationContainer { get; }
            ...
            Action<IMemberConfigurationExpression<TSource>> memberOptions)
        {
            ...
            var destProperty = memberInfo.ToMemberAccessor();
        ...
        {
            var typeInfo = new TypeDetails(TypeMap.DestinationType, ConfigurationContainer.ShouldMapProperty,
                ConfigurationContainer.ShouldMapField);
        ...
        {
            var mappingExpression = ConfigurationContainer.CreateMap<TDestination, TSource>(TypeMap.Profile,
                MemberList.Source);
        ...
        {
            var converter =
                new DeferredInstantiatedConverter<TSource, TDestination>(
                    BuildCtor<ITypeConverter<TSource, TDestination>>(typeof (TTypeConverter)));
            ...
            where TMappingAction : IMappingAction<TSource, TDestination>
        {
            Action<TSource, TDestination> beforeFunction =
                (src, dest) => ((TMappingAction) _serviceCtor(typeof (TMappingAction))).Process(src, dest);
        ...
        public IResolverConfigurationExpression<TSource, TValueResolver> ResolveUsing<TValueResolver>()
        ...
        {
            var baseTypeMap = ConfigurationContainer.CreateMap(sourceBase, destinationBase).TypeMap;
            baseTypeMap.IncludeDerivedTypes(typeof (TSource), typeof (TDestination));
            TypeMap.ApplyInheritedMap(baseTypeMap);
            return this;
        }

        protected IMappingExpression<TDestination, TSource> ConfigureReverseMap(
            IMappingExpression<TDestination, TSource> mappingExpression)
        {
        ...
        private void ForDestinationMember(IMemberAccessor destinationProperty,
            Action<IMemberConfigurationExpression<TSource>> memberOptions)
        {
