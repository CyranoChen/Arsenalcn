    ...
    public class Profile : IProfileExpression
    {
            ...
            get
            {
                if (!_memberConfigurations.Any())
                    _memberConfigurations.Add(
                        new MemberConfiguration().AddMember<NameSplitMember>()
        ...
        public void IncludeSourceExtensionMethods(Assembly assembly)
        {
            ...
            _sourceExtensionMethods.AddRange(assembly.GetTypes()
                .Where(type => TypeExtensions.IsSealed(type) && !TypeExtensions.IsGenericType(type) && !type.IsNested)
                .SelectMany(type => TypeExtensions.GetDeclaredMethods(type).Where(mi => mi.IsStatic))
